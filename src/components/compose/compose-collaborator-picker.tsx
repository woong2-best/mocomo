...
type Props = {
  ...
  max?: number;
  disabled?: boolean;
  /** toolbar: icon-only trigger (no chips) */
  compact?: boolean;
  /** selected chips only — pair with compact trigger in toolbar */
  chipsOnly?: boolean;
  labels?: {
    add?: string;
    search?: string;
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <ul className="max-h-64 space-y-1 overflow-y-auto">
          {pending && hits.length === 0 ? (
            <li className="px-2 py-4 text-center text-sm text-muted-foreground">…</li>
          ) : null}
          {hits.map((u) => {
              ...
              <li key={u.id}>
                ...
                >
                  <Avatar className="h-9 w-9">
                    <AvatarImage src={u.image ?? undefined} />
                    <AvatarFallback>{userDisplayName(u)[0]?.toUpperCase()}</AvatarFallback>
                  </Avatar>
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-xs text-muted-foreground">
                      @{u.username}
                      {u.isFollowing ? ` · ${labels?.following ?? "팔로잉"}` : ""}
                    </p>
                  </div>
