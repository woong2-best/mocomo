...
import { parseNaverLocalPart } from "@/lib/signup-email-domains";

type NaverLocalPartFieldProps = {
  value: string;
...
/** 네이버 메일 로컬 파트만 입력 — @naver.com 은 고정 표시 */
export function NaverLocalPartField({
  value,
  onChange,
  id = "naver-local",
  required = true,
  disabled = false,
  className,
}: NaverLocalPartFieldProps) {
  const { t } = useLocale();

  function handleChange(raw: string) {
    onChange(parseNaverLocalPart(raw.replace(/@/g, "")));
  }
  ...
  return (
    <div className={cn("space-y-1.5", className)}>
      ...
      <p className="text-[11px] text-muted-foreground">{t("auth.naverLocalHint")}</p>
    </div>
  );
}
