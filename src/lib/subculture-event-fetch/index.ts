...
function filterSeedFallback(
  ...
  const hasAnimeExpoAuto = autoKeys.has("auto-us-animeexpo-2026");
  const hasComicConAuto = autoKeys.has("auto-us-comiccon-2026");
  ...
  return seeds
    .filter((s) => {
      ...
      if (hasAnimeExpoAuto && s.externalKey.startsWith("official-us-anime-expo")) return false;
      if (hasComicConAuto && s.externalKey.startsWith("official-us-sdcc")) return false;
      return true;
    })
    .map(seedToFetched);
}
...
export async function fetchAllSubcultureEvents(): Promise<{
  ...
  const fallback = filterSeedFallback(ALL_SUBCULTURE_EVENT_SEEDS, auto);
