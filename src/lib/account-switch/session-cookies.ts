...
/** Drop leftover Auth.js callback so logout does not bounce back into a session. */
export function clearCallbackUrlCookiesOnResponse(res: NextResponse) {
...
export function clearSessionCookiesOnResponse(res: NextResponse) {
  ...
  clearCallbackUrlCookiesOnResponse(res);
}
...
async function clearCallbackUrlCookiesInJar() {
  const { cookies } = await import("next/headers");
  const jar = await cookies();
  for (const name of CALLBACK_URL_COOKIE_NAMES) {
    jar.set(name, "", CLEAR_COOKIE_OPTS);
...
export async function clearAllSessionCookies() {
  ...
  await clearCallbackUrlCookiesInJar();
}
