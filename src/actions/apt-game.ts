...
export async function purchaseAptSticker(typeId: string) {
  ...
  if (!("economy" in res)) return { error: "Íµ¨Îß§ Ï≤òÎ¶¨???§Ìå®?àÏäµ?àÎã§." as const };
...
export async function claimAptMission(missionId: string) {
  ...
  if (!mission) return { error: "ÎØ∏ÏÖò??Ï∞æÏùÑ ???ÜÏäµ?àÎã§." as const };
  if (!mission.completed) return { error: "?ÑÏßÅ ÎØ∏ÏÖò???ÑÎ£å?òÏ? ?äÏïò?µÎãà??" as const };
  if (mission.claimed) return { error: "?¥Î? Î≥¥ÏÉÅ??Î∞õÏïò?µÎãà??" as const };
...
export async function reportAptGameEvent(
  ...
  if (event.type === "place_sticker") {
    if (!canSpendEnergy(game.energy, ENERGY_COST_PLACE)) {
      return { error: `?êÎÑàÏßÄÍ∞Ä Î∂ÄÏ°±Ìï¥?? (??{ENERGY_COST_PLACE} ?ÑÏöî)` };
    }
