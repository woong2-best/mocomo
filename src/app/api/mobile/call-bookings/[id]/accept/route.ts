...
/** POST /api/mobile/call-bookings/[id]/accept — creator accepts booking */
export async function POST(
  ...
  const limited = await rateLimitPublicApi(req, `call-booking-accept:${auth.user.id}`, 30);
  ...
  if (booking.creatorId !== auth.user.id) {
    return NextResponse.json({ error: "크리에이터만 수락할 수 있습니다." }, { status: 403 });
  }
  if (booking.status !== "PENDING_CREATOR") {
    return NextResponse.json({ error: "수락할 수 없는 상태입니다." }, { status: 422 });
  }
  ...
  const updated = await db.creatorCallBooking.update({
    where: { id: booking.id },
    data: {
      status: "CONFIRMED",
