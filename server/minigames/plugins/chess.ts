...
import { pickChessAiMove } from "../../../src/lib/minigames/chess-ai";
...
function executeChessCpuTurn(room: MinigameRoomInternal) {
  if (!isCpuSoloRoom(room) || room.status !== "playing") return;
  const state = gs(room);
  if (turnUserId(state) !== MINIGAME_CPU_USER_ID) return;

  const ai = pickChessAiMove(state.fen, getRoomAiDifficulty(room));
  if (!ai) return;

  chessPlugin.applyMove(room, MINIGAME_CPU_USER_ID, {
    from: ai.from,
    to: ai.to,
  ...
  if (room.status === "playing") {
    (room as MinigameRoomInternal & { _broadcast?: () => void })._broadcast?.();
    if (turnUserId(gs(room)) === MINIGAME_CPU_USER_ID) scheduleChessCpuMoveIfNeeded(room);
  }
}

export function scheduleChessCpuMoveIfNeeded(room: MinigameRoomInternal) {
  if (!isCpuSoloRoom(room) || room.status !== "playing") return;
  if (turnUserId(gs(room)) !== MINIGAME_CPU_USER_ID) return;
  scheduleCpuTurn(room, () => executeChessCpuTurn(room));
}
